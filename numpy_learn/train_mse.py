...
net = Network(layers=[Linear(784, 128), ReLU(), Linear(128, 1),])
...
for epoch in progress_bar:
    ...
    while offset + batch_size <= len(x_val):
        ...
        offset += batch_size
    if (epoch) % 2 == 0:
        progress_bar.set_postfix({"Mean_loss_train": err, "Mean_loss_val": val_err})


test = pd.read_csv("mnist_test.csv", header=None).values[:, 1:]
test_label = pd.get_dummies(pd.read_csv("mnist_test.csv", header=None).values[:, 0])
